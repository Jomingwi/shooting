...
public class GameManager : MonoBehaviour
{
    ...
    IEnumerator sliderColorChange(bool _spawnBoss) // true가되면 보스가 출동해서 체력바로 사용함
    {
        float timer = 0.0f;

        while (timer < 1.0f)//조건이 참일때 반복
        {
    ...
    //yield return new WaitForSeconds(0.5f); //new ram 메모리를 할당받음
    ...
    private void checkTimer()
    {
        if (isSpawnBoss == false)
        {
            ...
            modifySlider();
            if (bossSpawnTimer >= bossSpawnTime) //시간변경이 완료되고 난뒤 보스가 출현
            {
                checkSpawnBoss();
            }
        }
    ...
    public void createItem(Vector3 _pos, Item.eItemType _type) // 0은 없음 1은 파워업 2는 체력회복
    {
    ...
    {
        score += _value;
        textScore.text = $"{score.ToString("d8")}"; // d8은 개수
    }
    ...
    public void GameOver()
    {
        ...
        for (int iNum = 0; iNum < count; iNum++)
        {
            ...
            if (userData.Score < score) //내점수가 랭크보다 높으면 해당랭크를 차지해야함
            {
                rank = iNum;
                break;
            }
        ...
        //textGameOverMenuBtn.text = rank != -1 ? "등록" : "메인 메뉴로";
        ...
        btnGameOverMenu.onClick.AddListener(() =>
        {
            if (rank != -1)
            {
                ...
                string value = JsonConvert.SerializeObject(listUserData);// 저장되어야할 내용
                PlayerPrefs.SetString(Tool.rankKey , value); // 저장할 키
            }
